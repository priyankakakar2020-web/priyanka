#!/usr/bin/env python3
"""
Utility script to pull factual data for JM Value Fund Direct Plan Growth
and supporting reference guides (e.g., capital gains statement instructions)
...
It also captures the official Groww blog instructions on downloading
capital-gains statements so the chatbot can explain the workflow with a
reliable citation.
"""
...
SCHEME_URL = "https://groww.in/mutual-funds/jm-basic-fund-direct-growth"
CAPITAL_GAINS_URL = (
    "https://groww.in/blog/how-to-get-capital-gains-statement-for-mutual-fund-investments"
)

ROOT = Path(__file__).resolve().parents[1]
SCHEME_OUTPUT = ROOT / "data" / "schemes" / "jm-value-fund-direct-plan-growth.json"
CAPITAL_GAINS_OUTPUT = ROOT / "data" / "guides" / "capital-gains-statement-groww.json"
...
class GuideMethod:
    ...
    def as_dict(self, source_url: str) -> Dict[str, Any]:
        return {
            "label": self.label,
            "steps": self.steps,
...
def extract_guide_methods(soup: BeautifulSoup) -> List[GuideMethod]:
    methods: List[GuideMethod] = []
    for list_tag in soup.find_all(["ul", "ol"]):
        items = [
            clean_text(li.get_text(" ", strip=True))
            for li in list_tag.find_all("li", recursive=False)
        ]
        if not items or not items[0].lower().startswith("step 1"):
            continue
        context_tag = list_tag.find_previous(["strong", "h2", "h3", "h4"])
        if not context_tag:
            continue
        context_text = clean_text(context_tag.get_text(" ", strip=True))
        methods.append(GuideMethod(label=context_text, steps=items))
    if not methods:
        raise ScrapeError("No step-by-step guides detected on capital gains page")
    return methods


def build_capital_gains_payload(url: str) -> Dict[str, Any]:
    soup = fetch_html(url)
    title_tag = soup.find("h1")
    title = clean_text(title_tag.get_text()) if title_tag else "Capital Gains Statement Guide"
    methods = extract_guide_methods(soup)
    timestamp = datetime.now(timezone.utc).isoformat()
    return {
        "guide_key": "capital-gains-statement-groww",
        "title": title,
        "source_url": url,
        "fetched_at": timestamp,
        "methods": [method.as_dict(url) for method in methods],
    }
...
def main() -> None:
    ...
    guide_payload = build_capital_gains_payload(CAPITAL_GAINS_URL)
    write_json(CAPITAL_GAINS_OUTPUT, guide_payload)
    print(
        "Saved scheme data to",
        SCHEME_OUTPUT.relative_to(ROOT),
        "and guide to",
        CAPITAL_GAINS_OUTPUT.relative_to(ROOT),
    )
